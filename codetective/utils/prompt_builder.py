...
class PromptBuilder:
    ...
    def build_prompt_from_config(
        ...
        if tone := config.get("style_or_tone"):
            prompt_parts.append(
                PromptBuilder.format_prompt_section("Follow these style and tone guidelines in your response:", tone)
            )

        if format_ := config.get("output_format"):
            prompt_parts.append(PromptBuilder.format_prompt_section("Structure your response as follows:", format_))
    ...
    def build_system_prompt_from_config(
        ...
        if constraints := config.get("output_constraints"):
            prompt_parts.append(PromptBuilder.format_prompt_section("Follow these important guidelines:", constraints))

        # Add style and tone guidelines
        if tone := config.get("style_or_tone"):
            prompt_parts.append(PromptBuilder.format_prompt_section("Communication style:", tone))

        # Add output format requirements
        if format_ := config.get("output_format"):
            prompt_parts.append(PromptBuilder.format_prompt_section("Response formatting:", format_))
