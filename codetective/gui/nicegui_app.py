...
class CodeDetectiveApp:
    ...
    def setup_sidebar(self):
        ...
        ui.button('📁 Project Selection', 
                 on_click=lambda: self.navigate_to('project_selection')).classes('w-full q-mb-sm')
        
        ui.button('🔍 Scan Results', 
                 on_click=lambda: self.navigate_to('scan_results')).classes('w-full q-mb-sm').bind_enabled_from(
                     self, 'scan_results', lambda x: x is not None)
        
        ui.button('🔧 Fix Application', 
                 on_click=lambda: self.navigate_to('fix_application')).classes('w-full q-mb-sm').bind_enabled_from(
                     self, 'selected_issues', lambda x: len(x) > 0)
        ...
        with self.project_config_container:
            ...
            with ui.row().classes('w-full'):
                ...
                with ui.column().classes('flex-1'):
                    ...
                    self.max_files_input = ui.number(
            ...
            ).classes('w-full q-mt-md').props('size=lg color=primary')
        ...
        try:
            ...
            with ui.scroll_area().classes('h-64 w-full max-w-full border'):
                self.file_tree = ui.tree(
                    tree_nodes,
                    label_key='label',
                    tick_strategy='leaf-filtered',
                    on_tick=self.on_tree_tick
                ).classes('w-full')
    ...
    async def start_scan(self):
        ...
        try:
            ...
            scan_result = await asyncio.get_event_loop().run_in_executor(
                None, orchestrator.run_scan, scan_config
            )
    ...
    def show_scan_results_page(self):
        ...
        if scan_result.total_issues > 0:
            ...
            ui.button('Select All Issues for Fixing', 
                     on_click=self.select_all_issues).classes('w-full q-mb-md')
            
            # Selected issues info and proceed button
            self.selected_issues_label = ui.label('').classes('text-info q-mb-md')
            self.proceed_button = ui.button('Proceed to Fix Application', 
                         on_click=lambda: self.navigate_to('fix_application')).classes('w-full').props('color=primary size=lg')
            ...
            with ui.expansion(f'{icon} {severity.upper()}: {issue.title}').classes('w-full q-mb-sm'):
                with ui.row().classes('w-full'):
                    ...
                    with ui.column().classes('flex-none'):
                        ...
                        checkbox = ui.checkbox('Include in fix', 
    ...
    def select_all_issues(self):
        ...
        all_issues = (self.scan_results.semgrep_results + 
    ...
    def show_fix_application_page(self):
        ...
        with ui.row().classes('w-full q-mb-md'):
            with ui.column().classes('flex-1'):
                self.fix_agent = ui.select(
                    ['edit', 'comment'],
                    label='Fix Agent',
                    value='edit'
                ).classes('w-full').style('min-width: 200px')
...
def main():
    ...
    create_app()
    ui.run(title='Codetective - Multi-Agent Code Review Tool', 
           port=7891, 
           show=True,
           reload=False)
